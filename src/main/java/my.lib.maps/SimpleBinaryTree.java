...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    public SimpleTreeNode getSimpleRoot() {
        return root;
    }
    ...
    private SimpleTreeNode fromBracketStr(String bracketStr, IndexWrapper iw, SimpleTreeNode backNode) throws Exception {
        ...
        if (bracketStr.charAt(iw.index) == '(') {
            ...
            skipSpaces(bracketStr, iw);
            if (bracketStr.charAt(iw.index) != ',') {
                parentNode.left = fromBracketStr(bracketStr, iw, parentNode.left);
                skipSpaces(bracketStr, iw);
            }
            if (bracketStr.charAt(iw.index) == ',') {
                iw.index++;
            ...
            if (bracketStr.charAt(iw.index) != ')') {
                parentNode.right = fromBracketStr(bracketStr, iw, parentNode.right);
                skipSpaces(bracketStr, iw);
            }
            if (bracketStr.charAt(iw.index) != ')') {
    ...
    public void fromBracketNotation(String bracketStr) throws Exception {
        ...
        SimpleTreeNode root = fromBracketStr(bracketStr, iw, null);
        if (iw.index < bracketStr.length()) {
            throw new Exception(String.format("Ожидался конец строки [%d]", iw.index));
        }
        this.root = root;
    }
    public SimpleBinaryTree<Integer> makeCopy(SimpleBinaryTree<Integer> majorTree)
    {
        SimpleBinaryTree<Integer> minorTree = new SimpleBinaryTree<>(Integer::parseInt);
        SimpleBinaryTree<Integer>.SimpleTreeNode nowNode = majorTree.getSimpleRoot();

        minorTree.root = rekursi(nowNode);

        return minorTree;
    }
    private SimpleBinaryTree<Integer>.SimpleTreeNode rekursi(SimpleBinaryTree<Integer>.SimpleTreeNode nowNode)
    {
        if(nowNode.getLeft() != null)
        {
            nowNode.left = rekursi(nowNode.left);
        }
        if(nowNode.getRight() != null)
        {
            nowNode.right = rekursi(nowNode.right);
        }
        return createNewNode(nowNode);
    }
    private SimpleBinaryTree<Integer>.SimpleTreeNode createNewNode(SimpleBinaryTree<Integer>.SimpleTreeNode node)
    {
        SimpleBinaryTree<Integer>.SimpleTreeNode newNode = (SimpleBinaryTree<Integer>.SimpleTreeNode) new SimpleTreeNode(null, null, null);
